...
def draw_lines(src, lines, what_line, mode = 'all', thickness=2):
    if mode == 'all':
        thickness = 2
...
def get_fitline(src, f_lines, size, what_line):
...
def get_fitline__(img, f_lines): # 대표선 구하기   
    lines = np.squeeze(f_lines)
...
def get_all_lines(src):
    ...
    if len(vertical_lines)!=0:
        ...
        vertical_fit_line = get_fitline(src, vertical_lines, size, 'vertical')
        answer[0] = 'vertical'
        answer[1] = vertical_fit_line[0]
        draw_lines(temp, vertical_lines, 'vertical')
        draw_lines(temp, vertical_fit_line, 'vertical', 'fit')
        src = cv2.addWeighted(src, 1, temp, 1., 0.)

    if len(horizontal_lines)!=0:
        print('horizontal_lines', len(horizontal_lines.shape)) 
        size = int(horizontal_lines.shape[0]*horizontal_lines.shape[2]/2)
        horizontal_fit_line = get_fitline(src, horizontal_lines, size, 'horizontal')
        answer[2] = 'horizontal'
        answer[3] = horizontal_fit_line[1]
        draw_lines(temp, horizontal_lines, 'horizontal')
        draw_lines(temp, horizontal_fit_line, 'horizontal', 'fit')
        src = cv2.addWeighted(src, 1, temp, 1., 0.)
    return answer, src

#-------------------------------------------------------------------

video = cv2.VideoCapture("./test__.mp4")
...
while True:
    ...
    answer, result = get_all_lines(src)
    print(answer)
